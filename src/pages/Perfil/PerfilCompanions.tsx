...
const PerfilCompanions: React.FC = () => {
			...
			.then(function (response: typeResponseAxios) {
				if (response.data.length > 0) {
					...
					setInfoAcompanhante({
						...
						linktree: profile.url_linktree,
						onlyfans: profile.url_onlyfans,
						web: profile.url_web,
						whatsapp: `55${profile.profile?.whatsapp.replace(/[-|(|)| ]/g, '')}`
					});
				}
			});
	}, []);
	...
	useEffect(() => {
		const checkUrl = (url: string, type: 'tiktok' | 'instagram' | 'twitter') => {
			if(!url.startsWith("@")){
				return url
			}
			let newUrl = url
...
export default PerfilCompanions;
